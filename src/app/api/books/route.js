...
export async function GET(request) {
    ...
    try {
        ...
        if (book_category === 'null') {
            ...
            const { rows, count } = await bookmodel.findAndCountAll({ where: { [Op.or]: { title: { [Op.like]: `%${name}%` } } }, offset: (page - 1) * 12, limit: 12, attributes: [`id`, `description`, `title`, `image`, `publish_date`, `author`, `slug`], order: [['serial_number', 'ASC']] });
            ...
            const { rows, count } = await bookmodel.findAndCountAll({ where: { book_category_id: id, [Op.or]: { title: { [Op.like]: `%${name}%` } } }, offset: (page - 1) * 12, limit: 12, attributes: [`id`, `description`, `title`, `image`, `publish_date`, `author`, `slug`], order: [['serial_number', 'ASC']] });
            return NextResponse.json({ status: true, booklist: rows, totalItems: count });
