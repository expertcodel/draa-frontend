...
JOIN (
  ...
  FROM courses c1
  JOIN (
    SELECT course_category_id, id
    FROM courses
    WHERE status = 1
    ORDER BY course_category_id, id
  ) c2 ON c1.course_category_id = c2.course_category_id AND c1.id = c2.id
  WHERE (
    SELECT COUNT(*) 
    FROM courses c3 
    WHERE c3.course_category_id = c1.course_category_id AND c3.id < c1.id AND c3.status = 1
  ) < 2
...
// SELECT course_categories.name AS category_name, course_categories.id AS id, CONCAT('[', GROUP_CONCAT(
//               '{',
